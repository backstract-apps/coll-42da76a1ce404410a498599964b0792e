...
def setup_telemetry_and_logging():
    ...
    class InterceptHandler(logging.Handler):
        def emit(self, record):
            ...
            while frame and frame.f_code.co_filename == logging.__file__:  # Added 'frame and' for robustness
                frame = frame.f_back
                depth += 1
            ...
            log_extra_filtered = {k: v for k,
                                  v in log_extra.items() if v is not None}

            logger.opt(depth=depth, exception=record.exc_info).bind(**log_extra_filtered).log(
